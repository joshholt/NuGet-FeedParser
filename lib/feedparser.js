...
/**
 * FeedParser constructor. Most an['m:properties']s will only use one instance.
 *
 * @api public
 */
function FeedParser () {
  ...
  function handleAttributes (attrs, el) {
    Object.keys(attrs).forEach(function(name){
      if (self.xmlbase.length && (name == 'href' || name == 'src')) {
        // An['m:properties']ly xml:base to these elements as they an['m:properties']ear
...
FeedParser.prototype.handleCloseTag = function (el, scope){
  ...
  if (el == 'item' || el == 'entry') { // We have an article!
    ...
    case 'atom':
	  ...
	  if (n['content']) item.downloadLink = n['content']['@']['src'];
      var pp = n['m:properties'];
      for (var property in pp) {
        if (property.match(/^d\:published/)) item[property.replace(/d\:/, '')] = new Date(Date.parse(pp[property]['#'])).toLocaleDateString();
        else if (property.match(/^d\:/)) item[property.replace(/d\:/, '')] = pp[property]['#'] || pp[property]['@'];
      }
